...
func executeIntcodeProgram(_ program: [Int]) -> [Int] {
    var pc = 0
    var result = program
    while result[pc] != 99 {
        switch result[pc] {
...
testCasesPart1.forEach { input, expectedOutput in
    let output = executeIntcodeProgram(input)
...
func determinCorrectInputs(_ program: [Int]) -> (noun: Int, verb: Int)? {
    let targetOutput = 19690720
    for noun in 0 ... 99 {
        for verb in 0 ... 99 {
            var challengeInputPart2 = challengeInput
            challengeInputPart2[1] = noun
            challengeInputPart2[2] = verb
            let outputPart2 = executeIntcodeProgram(challengeInputPart2)
            if outputPart2[0] == targetOutput {
                return (noun, verb)
            }
        }
    }

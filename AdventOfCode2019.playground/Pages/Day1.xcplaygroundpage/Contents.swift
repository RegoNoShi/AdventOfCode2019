...
func fuelRequired(forFuel fuel: Int) -> Int {
    var totalFuelRequired = fuel
    var lastFuelAdded = fuel
    while true {
        let additionalFuel = fuelRequired(forMass: lastFuelAdded)
        if additionalFuel <= 0 {
            break
        } else {
            totalFuelRequired += additionalFuel
            lastFuelAdded = additionalFuel
        }
    }
